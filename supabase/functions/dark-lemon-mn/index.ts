...
## REGOLA OPERATIVA CERNITE, GIACENZE E REGISTRO (verificata con prova reale 11/09/2026)
...
- STAMPA REGISTRO PER PERIODO (dall'11/09/2026): nel Registro Generale ci sono i campi "Dal" e "al". Impostato l'intervallo, i pulsanti Excel e "Stampa PDF (periodo)" esportano SOLO i movimenti di quelle date, con intestazione registro + periodo. Il filtro per singolo giorno resta disponibile e si combina con l'intervallo.
- CONTRATTI A CANONE (dall'11/09/2026): scheda "Contratti a canone" nel modulo Fatturazione (tabelle contratti_clienti, contratti_clienti_righe, contratti_fatture_generate). Le voci marcate ricorrenti (noleggi cassoni, compattatori, press-container) generano ogni mese una fattura in bozza con il pulsante "Genera fatture del mese" (funzione genera_fatture_contratti(tenant, periodo)). Lo stesso mese non può essere fatturato due volte; con il pulsante "Recesso" il contratto smette di generare fatture. Le voci non ricorrenti (smaltimento, trasporto a kg) restano listino e NON generano canoni.
- ALLINEAMENTO UFFICIALE 11/09/2026: giacenze Multyproget e registri (Impianto, Conto Proprio, Niyol) riallineati alla stampa registrazioni per CER e ai registri ufficiali del cliente; le rettifiche sono tracciate in movimenti_impianto con origine 'allineamento_ufficiale'.
...
## 📘 MANUALE OPERATIVO DEV MULTY (rif. guida completa)
...
- Ogni bozza è eliminabile con il **cestino** (soft delete). Una bozza non modifica giacenze; i movimenti effettivi richiedono uno storno umano tracciato.
