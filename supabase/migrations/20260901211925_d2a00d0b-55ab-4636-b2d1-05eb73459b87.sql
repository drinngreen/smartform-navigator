...
BEGIN
  ...
  -- 4
  SELECT 2 - count(*) INTO n FROM pg_trigger
   WHERE NOT tgisinternal AND tgname IN ('trg_dragon_sync_stock_to_magazzino','trg_magazzino_sync_to_dragon');
  res := res || jsonb_build_object('check','Controlli automatici di allineamento attivi','anomalie',greatest(n,0));
  ...
  -- 9
  SELECT count(*) INTO n FROM privati_conferimenti pc
   WHERE pc.data >= '2026-01-01'
     AND NOT EXISTS (
       SELECT 1 FROM rentri_registro_esiti e
        WHERE e.registro_label = 'MULTY_PRIVATI'
          AND e.numero_interno = pc.numero_progressivo
          AND e.data_movimento = pc.data::date);
