-- 1. Additive columns
ALTER TABLE public.dragon_transform_batches ADD COLUMN IF NOT EXISTS calo_peso_kg numeric NOT NULL DEFAULT 0;
ALTER TABLE public.dragon_transform_batches ADD COLUMN IF NOT EXISTS test_session text;
ALTER TABLE public.dragon_register_movements ADD COLUMN IF NOT EXISTS test_session text;
ALTER TABLE public.dragon_stock_movements ADD COLUMN IF NOT EXISTS test_session text;
ALTER TABLE public.dragon_items ADD COLUMN IF NOT EXISTS test_session text;
ALTER TABLE public.dragon_lots ADD COLUMN IF NOT EXISTS test_session text;
ALTER TABLE public.dragon_documents ADD COLUMN IF NOT EXISTS test_session text;

CREATE INDEX IF NOT EXISTS idx_dragon_reg_mov_test ON public.dragon_register_movements(test_session) WHERE test_session IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_dragon_stock_mov_test ON public.dragon_stock_movements(test_session) WHERE test_session IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_dragon_items_test ON public.dragon_items(test_session) WHERE test_session IS NOT NULL;

-- 2. Calo peso auto-maintained
CREATE OR REPLACE FUNCTION public.dragon_recalc_calo_peso()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  UPDATE public.dragon_transform_batches b
     SET calo_peso_kg = GREATEST(b.input_quantity - COALESCE((
           SELECT SUM(o.output_quantity) FROM public.dragon_transform_batch_outputs o WHERE o.batch_id = b.id
         ), 0), 0)
   WHERE b.id = v_batch;
  RETURN NULL;
END;
...
DROP TRIGGER IF EXISTS trg_dragon_calo_peso ON public.dragon_transform_batch_outputs;
CREATE TRIGGER trg_dragon_calo_peso
AFTER INSERT OR UPDATE OR DELETE ON public.dragon_transform_batch_outputs
FOR EACH ROW EXECUTE FUNCTION public.dragon_recalc_calo_peso();

UPDATE public.dragon_transform_batches b
   SET calo_peso_kg = GREATEST(b.input_quantity - COALESCE((
         SELECT SUM(o.output_quantity) FROM public.dragon_transform_batch_outputs o WHERE o.batch_id = b.id), 0), 0)
 WHERE b.status = 'CONFERMATA';

-- 3. Snapshot helper (ignores test rows)
CREATE OR REPLACE FUNCTION public.dragon_test_balance_snapshot(p_company_id uuid)
RETURNS jsonb
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT COALESCE(jsonb_object_agg(k, v), '{}'::jsonb) FROM (
    SELECT item_id::text || '|' || warehouse_scope::text AS k,
           SUM(CASE WHEN sign = 'PLUS' THEN quantity ELSE -quantity END) AS v
      FROM public.dragon_stock_movements
     WHERE company_id = p_company_id AND test_session IS NULL
     GROUP BY 1
  ) t;
$$;
...
BEGIN
  ...
  DELETE FROM public.fir_forms
   WHERE tenant_id = p_company_id AND numero_fir LIKE 'ZTEST%' AND note_aggiuntive = p_session;
...
-- 5. Tag helper
CREATE OR REPLACE FUNCTION public.dragon_test_tag_batch(p_company_id uuid, p_batch_id uuid, p_session text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $$
BEGIN
  UPDATE public.dragon_transform_batches SET test_session = p_session WHERE id = p_batch_id AND company_id = p_company_id;
  UPDATE public.dragon_register_movements SET test_session = p_session
   WHERE company_id = p_company_id AND source_transform_batch_id = p_batch_id AND test_session IS NULL;
  UPDATE public.dragon_stock_movements SET test_session = p_session
   WHERE company_id = p_company_id AND (source_transform_batch_id = p_batch_id
      OR source_register_movement_id IN (SELECT id FROM public.dragon_register_movements WHERE test_session = p_session))
     AND test_session IS NULL;
  UPDATE public.dragon_lots SET test_session = p_session
   WHERE company_id = p_company_id AND test_session IS NULL
     AND id IN (SELECT lot_id FROM public.dragon_transform_batch_outputs WHERE batch_id = p_batch_id AND lot_id IS NOT NULL);
END;
$$;
...
BEGIN
  ...
  IF p_scenario = 'fir' THEN
    ...
    INSERT INTO public.fir_forms (user_id, tenant_id, status, numero_fir, note_aggiuntive)
    VALUES (auth.uid(), p_company_id, 'bozza', 'ZTEST' || to_char(now(),'HH24MISS'), v_session)
    ...
    BEGIN
      INSERT INTO public.fir_forms (user_id, tenant_id, status, numero_fir, note_aggiuntive)
      VALUES (auth.uid(), p_company_id, 'bozza', (SELECT numero_fir FROM public.fir_forms WHERE id = v_fir), v_session);
    EXCEPTION WHEN OTHERS THEN
      v_ok := true; v_msg := SQLERRM;
    END;
...
REVOKE ALL ON FUNCTION public.dragon_test_tag_batch(uuid, uuid, text) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.dragon_test_balance_snapshot(uuid) FROM PUBLIC, anon;
...
GRANT EXECUTE ON FUNCTION public.dragon_test_tag_batch(uuid, uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.dragon_test_balance_snapshot(uuid) TO authenticated;