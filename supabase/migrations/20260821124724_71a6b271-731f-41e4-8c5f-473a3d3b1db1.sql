...
DECLARE
  ...
  v_n integer;
BEGIN
  IF NOT (auth.role() = 'service_role' OR (auth.uid() IS NOT NULL AND public.can_access_tenant(p_company_id))) THEN
...
DECLARE
  ...
  v_diff jsonb;
BEGIN
  IF p_company_id IS NULL OR NOT (auth.role() = 'service_role' OR (auth.uid() IS NOT NULL AND public.can_access_tenant(p_company_id))) THEN
    RAISE EXCEPTION 'Accesso non autorizzato';
  END IF;
  ...
  v_actor := COALESCE(auth.uid(), (SELECT user_id FROM public.profiles WHERE tenant_id = p_company_id ORDER BY created_at LIMIT 1));
  ...
  IF p_scenario IN ('cernite','giacenze') THEN
    ...
    VALUES (p_company_id, v_register, CURRENT_DATE, CURRENT_DATE, v_padre, 'ZT9901', 'TEST metalli misti',
            'CARICO', v_cause_in, 1000, 'PLUS', 'CONSOLIDATO', v_actor, v_session, 'Movimento di test ' || v_session)
  ...
  IF p_scenario = 'giacenze' THEN
    INSERT INTO public.dragon_register_movements
      (company_id, register_id, movement_date, recording_date, item_id, cer_code, description_snapshot,
       movement_type, cause_id, quantity, sign, status, created_by, test_session, note)
    VALUES (p_company_id, v_register, CURRENT_DATE, CURRENT_DATE, v_padre, 'ZT9901', 'TEST metalli misti',
            'SCARICO', v_cause_out, 400, 'MINUS', 'CONSOLIDATO', v_actor, v_session, 'Scarico di test ' || v_session)
    RETURNING id INTO v_reg;
    UPDATE public.dragon_stock_movements SET test_session = v_session WHERE source_register_movement_id = v_reg;
    v_bal := public.dragon_get_stock_balance(p_company_id, v_padre, 'WASTE');
  ...
  IF p_scenario = 'fir' THEN
    ...
    VALUES (v_actor, p_company_id, 'bozza', v_num, v_session)
...
GRANT EXECUTE ON FUNCTION public.dragon_test_run(uuid, text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.dragon_test_cleanup(uuid, text) TO authenticated, service_role;