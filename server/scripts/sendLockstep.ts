...
function logEvent(e:any){ try { ensureOut(); appendFileSync('out/stream-log.jsonl', JSON.stringify(e)+'\n') } catch {} }
...
async function sendAndCheck(registryId:string, filename:string){
  ...
  let list = await listMovimenti(registryId, filename, issuer, 50)
  let head = list[0]
  if (!head) {
    try {
  ...
  if (!head) return { ok:false, reason:'no_head' }
  ...
  if (status !== 202) {
    logEvent({ ts: Date.now(), registryId, filename, count: 1, accepted: 0, duplicates: 0, errore: true, http: status })
    return { ok:false, status, progressivo }
  }
  let tid = ''
  ...
  for (let i=0;i<20;i++){
    ...
    try {
      if (chk.data?.success && Number(chk.data?.status)===200) {
        ...
        logEvent({ ts: Date.now(), registryId, filename, count: 1, accepted: acc, duplicates: dup, errore: !!body?.errore })
  ...
  logEvent({ ts: Date.now(), registryId, filename, count: 1, accepted: 0, duplicates: 0, errore: true })
...
async function main(){
  for (let attempt=1; attempt<=10; attempt++){
    const g = await sendAndCheck('R6QSWHZ6HJV','certificato.p12')
    const m = await sendAndCheck('RQEL39R7NS0','multyproget.p12')
    if (g.ok && m.ok) {
      const gOk = await confirmOnRegistrazioni('R6QSWHZ6HJV','certificato.p12', String(g.progressivo||''))
      const mOk = await confirmOnRegistrazioni('RQEL39R7NS0','multyproget.p12', String(m.progressivo||''))
      console.log(JSON.stringify({ attempt, status:'pair', global: { progressivo: g.progressivo, confirmed: gOk }, multy: { progressivo: m.progressivo, confirmed: mOk } }))
      if (gOk && mOk) return
    } else {
      console.log(JSON.stringify({ attempt, status:'retry', global: { ok: g.ok, reason: (g as any).reason }, multy: { ok: m.ok, reason: (m as any).reason } }))
    }
    await sleep(3000)
  }
