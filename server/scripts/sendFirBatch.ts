...
function buildPayloadFromFir(fir:any, isMulty:boolean, idx:number, base:number, annoOverride?:number){
  const cf = String(fir?.identificativi?.codiceFIR||'')
  let prog = normInt(cf)
  const isPlaceholder = /FMGWB/i.test(cf) || prog < 100000
  if (isPlaceholder || !prog || prog===0) prog = (base + (idx+1))
  const anno = Number(annoOverride || new Date(String(fir?.identificativi?.dataEmissione||new Date().toISOString().slice(0,10))).getFullYear())
...
async function runFor(cfg:{registryId:string; filename:string; issuer:string}, isMulty:boolean, fir:any, idx:number, base:number, anno?:number){
  const payload = [buildPayloadFromFir(fir, isMulty, idx, base, anno)]
  ...
  logLine({ kind:'SEND', registryId:cfg.registryId, status: sent.status, transazioneId: sent.transazioneId, key })
  if (sent.status === 202 && sent.transazioneId){
    ...
    if (!bodyStr || acc===0) logLine({ kind:'RESULT_PENDING', registryId:cfg.registryId, transazioneId: sent.transazioneId, accepted: 0 })
...
async function main(){
  ...
  const batch = all.slice(0, Math.max(1, limit))
  for (let i=0;i<batch.length;i++){
    const fir = batch[i]
    await runFor(GLOBAL, false, fir, i, base, anno)
    if (!onlyGlobal) await runFor(MULTY, true, fir, i, base, anno)
  }
}
main()
