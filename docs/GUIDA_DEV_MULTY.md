...
- **Codice SDI (fatturazione elettronica)**: nella scheda soggetto (rubrica e pulsante NUOVO SOGGETTO dal formulario) c'è il campo **Codice SDI (fatturazione)**, accanto alla PEC. La PEC **non è obbligatoria**: se il cliente comunica solo il codice destinatario SDI (6 o 7 caratteri) va scritto lì, non nella PEC e non nelle note. Il codice viene salvato in `anagrafica_aziende_mp.codice_destinatario` ed è quello usato dal modulo Fatturazione per l'invio elettronico.
- **Sosta tecnica (punto 15 del FIR cartaceo)**: nel formulario Standard la sezione **15. Sosta tecnica (sospensione del trasporto)** si trova nella prima pagina, subito prima delle Annotazioni (luogo, inizio e fine sospensione). Eventuali seconda e terza sosta si compilano nella pagina "Trasbordo / Soste". Non vanno più scritte nelle Annotazioni.
- **Stampa del registro per periodo**: nel Registro Generale, accanto ai filtri, ci sono i campi **Dal** e **al**. Scelte le due date, i pulsanti **Excel** e **Stampa PDF (periodo)** producono solo i movimenti di quell'intervallo, con in testa il registro selezionato e il periodo stampato. La X accanto ai campi azzera l'intervallo.
- **Contratti a canone (fatture automatiche ogni mese)**: nel modulo Fatturazione c'è la scheda **Contratti a canone**. Ogni contratto elenca le sue voci: quelle con il simbolo di ripetizione sono i canoni (noleggio cassoni, compattatori, press-container), le altre sono il listino a consumo (smaltimento e trasporto a kg). Scegliendo mese e anno e premendo **Genera fatture del mese** il programma crea una fattura in bozza per ogni contratto attivo; lo stesso mese non viene mai fatturato due volte. Con **Recesso** il contratto si ferma e non produce più fatture; **Riattiva** lo rimette in funzione.
