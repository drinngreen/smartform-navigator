...
interface GeoPos { lat: number; lng: number; speed: number | null; accuracy: number | null; }
...
export default function MNAppGPSPage() {
  ...
  useEffect(() => {
    ...
    const wid = navigator.geolocation.watchPosition(
      (pos) => setPosition({ lat: pos.coords.latitude, lng: pos.coords.longitude, speed: pos.coords.speed, accuracy: pos.coords.accuracy }),
    ...
    return () => {
      ...
      watchIdRef.current = null; intervalRef.current = null;
    };
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 pb-20 space-y-4">
        <div className="p-4 rounded-2xl bg-card/60 border border-border/30 backdrop-blur-xl">
          <div className="flex items-center gap-3 mb-3">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center ${tracking ? 'bg-neon-green/20' : 'bg-destructive/20'}`}>
              {tracking ? <Signal className="h-5 w-5 text-neon-green" /> : <WifiOff className="h-5 w-5 text-destructive" />}
            </div>
            <div>
        ...
        <div className="p-4 rounded-2xl bg-card/60 border border-primary/20">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between"><span className="text-xs text-muted-foreground">Targa</span><span className="text-sm font-mono font-semibold text-foreground">{profile?.targa_automezzo || "—"}</span></div>
            <div className="flex items-center justify-between"><span className="text-xs text-muted-foreground">Stato</span>
              <span className={`text-xs font-mono px-2 py-0.5 rounded border ${tracking && position?.speed && position.speed > 1 ? "bg-neon-green/20 text-neon-green border-neon-green/30" : "bg-secondary/50 text-muted-foreground border-border/30"}`}>
