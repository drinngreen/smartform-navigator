...
export default function MNAppGPSPage() {
    ...
    useEffect(() => {
        if (optedOut || !navigator.geolocation || !user?.id)
            return;
        const wid = navigator.geolocation.watchPosition((pos) => setPosition({ lat: pos.coords.latitude, lng: pos.coords.longitude, speed: pos.coords.speed, accuracy: pos.coords.accuracy }), () => { }, { enableHighAccuracy: true, timeout: 10000 });
        watchIdRef.current = wid;
        const sendPosition = (pos) => {
            supabase.from("driver_locations").insert({ user_id: user.id, lat: pos.coords.latitude, lng: pos.coords.longitude, speed: pos.coords.speed, accuracy: pos.coords.accuracy, tenant_id: profile?.tenant_id || null }).then(({ error }) => { if (error)
