...
export default function MNAppComunicazioniPage() {
  ...
  const handleSend = async () => { if (!newMessage.trim()) return; try { await sendMessage(newMessage.trim()); setNewMessage(""); } catch {} };
      ...
      <div className="flex flex-col h-full min-h-screen">
        <div className="px-4 pt-4 pb-2 flex items-center justify-between" style={{ borderBottom: '1px solid rgba(192, 173, 103, 0.15)' }}>
          <div>
            ...
            <p className="text-muted-foreground text-xs font-mono mt-1 flex items-center gap-1"><MessageCircle className="h-3 w-3" /> Chat con {context === "niyol" ? "Niyol" : "Multyproget"}</p>
          </div>
        ...
        <div className="px-4 py-3" style={{ borderBottom: '1px solid rgba(192, 173, 103, 0.15)' }}>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-secondary/50 border border-primary/30 flex items-center justify-center"><span className="text-sm font-display font-bold text-foreground">{context === "niyol" ? "N" : "M"}</span></div>
            <div><p className="text-sm font-display font-semibold text-foreground">{context === "niyol" ? "Niyol" : "Multyproget"}</p><p className="text-xs text-neon-green font-mono">Online</p></div>
          </div>
        </div>
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
          {isLoading ? (<div className="flex items-center justify-center py-12"><div className="text-primary animate-pulse font-display">Caricamento...</div></div>
          ) : (!messages || messages.length === 0) ? (
            <div className="flex flex-col items-center justify-center py-16 text-center"><p className="text-muted-foreground text-sm">Nessun messaggio</p><p className="text-muted-foreground/60 text-xs mt-1">Inizia la conversazione!</p></div>
          ) : (
