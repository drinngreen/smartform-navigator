  ...
  return (
    <MobileShell>
      ...
      <div className="px-4 py-2 flex items-center justify-between border-b border-border/20">
        ...
        <button onClick={() => navigate(basePath)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-border/30 text-xs text-muted-foreground hover:text-foreground transition-colors">
            ...
            <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[80%] p-3 rounded-2xl text-sm ${msg.role === "user" ? "bg-primary/20 text-foreground rounded-br-sm" : "bg-card/60 border border-border/30 text-foreground rounded-bl-sm"}`}>
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-[10px] text-muted-foreground font-mono">{msg.role === "user" ? "Tu" : "Dragon AI"}</span>
                </div>
