...
import { ArrowLeft, ChevronLeft, ChevronRight, CheckCircle2, Circle, Maximize2, X, RotateCcw, BookOpen, AlertTriangle, HelpCircle, ListChecks, } from "lucide-react";
const CHAPTERS = [
    {
        id: "orientamento",
        ...
        level: "Base",
        minutes: 3,
        intro: "Il Centro di Comando Dev Multy è un'unica pagina con tante tab. Ogni tab è un contesto isolato: quello che vedi in una tab non contamina le altre. Prima di imparare i singoli moduli conviene capire come è organizzato il lavoro.",
        explain: [
    ...
    },
    {
        ...
        level: "Base",
        minutes: 6,
        intro: "Il Modulo Standard è la vista 'a schede' del formulario: gli stessi dati del modulo cartaceo, ma organizzati in sezioni ordinate. È il punto in cui si crea la maggior parte del lavoro quotidiano.",
        explain: [
    ...
    },
    {
        ...
        level: "Base",
        minutes: 3,
        intro: "Il Modulo Alternativo riproduce fedelmente il formulario ufficiale in formato stampabile, con i campi trasparenti sovrapposti al modulo.",
        explain: [
    ...
    },
    {
        ...
        level: "Base",
        minutes: 5,
        intro: "Gli autisti Multyproget e Niyol usano un'app mobile dedicata, con lo stesso database del gestionale ma un'interfaccia ridotta all'essenziale: compilare il formulario, tracciare il viaggio, comunicare con l'ufficio.",
        explain: [
    ...
    },
    {
        ...
        level: "Intermedio",
        minutes: 3,
        intro: "La Cronologia dell'app è lo specchio del lavoro dell'autista: bozze aperte da completare e formulari già inviati.",
        explain: [
    ...
    },
    {
        ...
        level: "Intermedio",
        minutes: 4,
        intro: "La tab Impianto mostra esclusivamente i movimenti dell'impianto Multyproget di via Rivarossa. Ogni formulario in cui Multyproget è Produttore o Destinatario genera qui un movimento di CARICO o SCARICO.",
        explain: [
    ...
    },
    {
        ...
        level: "Intermedio",
        minutes: 3,
        intro: "Niyol è la società di trasporto. Se Niyol trasporta per Multyproget, il formulario è visibile in entrambi i contesti, ma le giacenze si muovono solo su Multyproget.",
        explain: [
    ...
    },
    {
        ...
        level: "Intermedio",
        minutes: 3,
        intro: "In Conto Proprio gestisci i formulari dove Multyproget trasporta i propri rifiuti con mezzi propri (categoria 2-bis).",
        explain: [
    ...
    },
    {
        ...
        level: "Avanzato",
        minutes: 5,
        intro: "I registri sono cronologici e isolati per società. Da qui prepari gli invii verso RENTRI e gestisci i casi particolari (formulari cartacei, lavorazioni R13).",
        explain: [
    ...
    },
    {
        ...
        level: "Base",
        minutes: 5,
        intro: "Il registro Privati gestisce i conferimenti dei cittadini. Ogni conferimento può contenere più materiali e l'inserimento è atomico: o si registra tutto (conferimento + movimenti + giacenze), o non si registra nulla.",
        explain: [
    ...
    },
    {
        ...
        level: "Base",
        minutes: 3,
        intro: "Ogni conferimento genera una ricevuta con numero progressivo annuale univoco (es. 258/2026), con tutti i materiali conferiti.",
        explain: [
    ...
    },
    {
        ...
        level: "Intermedio",
        minutes: 4,
        intro: "La rubrica alimenta i menu a tendina dei formulari. Dati corretti qui significano formulari compilati in un click.",
        explain: [
    ...
    },
    {
        ...
        level: "Intermedio",
        minutes: 4,
        intro: "Le giacenze sono sempre il risultato dei movimenti reali: non esiste un saldo iniziale digitato a mano.",
        explain: [
    ...
    },
    {
        ...
        level: "Avanzato",
        minutes: 6,
        intro: "Il modulo Fatturazione è un mini-ERP contabile isolato dal resto: gestisce fatture di vendita, anagrafiche clienti, piano dei conti, tabelle fiscali e prima nota.",
        explain: [
    ...
    },
    {
        ...
        level: "Avanzato",
        minutes: 6,
        intro: "Sibill è il provider di fatturazione elettronica collegato al gestionale: prende la fattura creata qui dentro, la trasforma in XML FatturaPA e la trasmette allo SDI, riportando indietro gli esiti.",
        explain: [
    ...
    },
    {
        ...
        level: "Intermedio",
        minutes: 4,
        intro: "Da qui crei, modifichi ed elimini i login degli autisti, decidendo a quale app assegnarli: Multyproget oppure Niyol.",
        explain: [
    ...
    },
    {
        ...
        level: "Intermedio",
        minutes: 3,
        intro: "Ogni numero FIR va assegnato manualmente: a un autista oppure contrassegnato come 'usato dall'ufficio'.",
        explain: [
    ...
    },
    {
        ...
        level: "Avanzato",
        minutes: 5,
        intro: "La console mostra lo stato del ponte verso il RENTRI, i numeri FIR scaricati e i formulari in attesa di firma digitale.",
        explain: [
    ...
    },
    {
        ...
        level: "Avanzato",
        minutes: 8,
        intro: "Dark Lemon è l'assistente intelligente integrato nel gestionale: non è una semplice chat, è un agente che legge davvero i dati dell'azienda, controlla la coerenza di giacenze, formulari e conferimenti, e può eseguire azioni operative al posto tuo.",
        explain: [
...
export default function MNDevTutorialPage() {
    ...
    const completed = useMemo(() => CHAPTERS.filter((c) => c.steps.every((_, i) => done[`${c.id}:${i}`])).length, [done]);
    const progress = Math.round((completed / CHAPTERS.length) * 100);
    const totalMinutes = useMemo(() => CHAPTERS.reduce((a, c) => a + c.minutes, 0), []);
    const toggleStep = (i) => setDone((d) => ({ ...d, [`${chapter.id}:${i}`]: !d[`${chapter.id}:${i}`] }));
    const goTo = (i) => {
        setIndex(i);
        setZoom(false);
        window.scrollTo({ top: 0, behavior: "smooth" });
    };
    return (_jsxs(MNAdminLayout, { title: "\uD83C\uDFAC Tutorial Interattivo", subtitle: "Multyproget \u00B7 Impara il gestionale passo dopo passo", children: [_jsxs("div", { className: "mb-4 flex flex-wrap items-center justify-between gap-3", children: [_jsxs(Button, { variant: "ghost", onClick: () => navigate("/mn/admin/dev-multyproget"), className: "gap-2 text-muted-foreground hover:text-foreground", children: [_jsx(ArrowLeft, { size: 16 }), " Torna al Centro di Comando"] }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsxs(Button, { variant: "ghost", size: "sm", className: "gap-2 text-muted-foreground", onClick: () => setDone({}), children: [_jsx(RotateCcw, { size: 14 }), " Azzera progressi"] }), _jsxs(Button, { variant: "outline", size: "sm", className: "gap-2", onClick: () => navigate("/mn/admin/dev-multyproget/guida"), children: [_jsx(BookOpen, { size: 14 }), " Guida testuale"] })] })] }), _jsxs("div", { className: "mb-5 rounded-xl border border-border/40 bg-card/60 p-4 backdrop-blur-xl", children: [_jsxs("div", { className: "mb-2 flex flex-wrap items-center justify-between gap-2 text-xs font-mono uppercase tracking-wider text-muted-foreground", children: [_jsxs("span", { children: ["Avanzamento tutorial \u00B7 percorso completo ~", totalMinutes, " min"] }), _jsxs("span", { children: [completed, "/", CHAPTERS.length, " capitoli \u00B7 ", progress, "%"] })] }), _jsx("div", { className: "h-2 w-full overflow-hidden rounded-full bg-secondary", children: _jsx("div", { className: "h-full rounded-full bg-emerald-500 transition-all", style: { width: `${progress}%` } }) })] }), _jsxs("div", { className: "grid gap-5 lg:grid-cols-[250px_1fr]", children: [_jsx("nav", { className: "flex gap-2 overflow-x-auto lg:flex-col lg:overflow-visible", children: CHAPTERS.map((c, i) => {
                            const cDone = c.steps.every((_, s) => done[`${c.id}:${s}`]);
                            return (_jsxs("button", { onClick: () => goTo(i), className: `flex min-w-[190px] items-center gap-2 rounded-lg border px-3 py-2 text-left text-sm transition-all lg:min-w-0 ${i === index
                                    ? "border-emerald-500/50 bg-emerald-500/15 text-emerald-300"
                                    : "border-border/40 bg-card/40 text-muted-foreground hover:bg-secondary/50"}`, children: [cDone ? _jsx(CheckCircle2, { size: 15, className: "shrink-0 text-emerald-400" }) : _jsx(Circle, { size: 15, className: "shrink-0 opacity-50" }), _jsxs("span", { className: "truncate", children: [i + 1, ". ", c.title] })] }, c.id));
                        }) }), _jsxs("section", { className: "space-y-4", children: [_jsxs("div", { className: "rounded-xl border border-border/40 bg-card/60 p-5 backdrop-blur-xl", children: [_jsxs("div", { className: "flex flex-wrap items-center gap-2", children: [_jsxs("p", { className: "text-xs font-mono uppercase tracking-wider text-emerald-400", children: ["Capitolo ", index + 1, " di ", CHAPTERS.length] }), _jsx("span", { className: `rounded-full border px-2 py-0.5 text-[10px] font-mono uppercase tracking-wider ${LEVEL_STYLE[chapter.level]}`, children: chapter.level }), _jsxs("span", { className: "rounded-full border border-border/40 bg-secondary/30 px-2 py-0.5 text-[10px] font-mono uppercase tracking-wider text-muted-foreground", children: ["~", chapter.minutes, " min"] })] }), _jsx("h2", { className: "mt-1 text-2xl font-display font-bold text-foreground", children: chapter.title }), _jsx("p", { className: "text-sm text-muted-foreground", children: chapter.subtitle }), _jsx("p", { className: "mt-3 text-sm leading-relaxed text-foreground/90", children: chapter.intro })] }), _jsxs("div", { className: "group relative overflow-hidden rounded-xl border border-border/40 bg-black/30", children: [_jsx("img", { src: chapter.image, alt: `Schermata ${chapter.title}`, loading: "lazy", className: "w-full object-cover" }), _jsx("button", { onClick: () => setZoom(true), className: "absolute right-3 top-3 rounded-lg border border-border/60 bg-background/80 p-2 text-foreground backdrop-blur transition-all hover:bg-background", "aria-label": "Ingrandisci schermata", children: _jsx(Maximize2, { size: 16 }) })] }), _jsxs("div", { className: "rounded-xl border border-border/40 bg-card/60 p-5 backdrop-blur-xl", children: [_jsx("h3", { className: "mb-3 text-sm font-display font-bold uppercase tracking-wider text-foreground", children: "Come funziona" }), _jsx("div", { className: "space-y-3", children: chapter.explain.map((p, i) => (_jsx("p", { className: "text-sm leading-relaxed text-foreground/85", children: p }, i))) })] }), chapter.fields && (_jsxs("div", { className: "rounded-xl border border-border/40 bg-card/60 p-5 backdrop-blur-xl", children: [_jsxs("h3", { className: "mb-3 flex items-center gap-2 text-sm font-display font-bold uppercase tracking-wider text-foreground", children: [_jsx(ListChecks, { size: 15, className: "text-cyan-400" }), " I campi, uno per uno"] }), _jsx("dl", { className: "grid gap-2 sm:grid-cols-2", children: chapter.fields.map((f) => (_jsxs("div", { className: "rounded-lg border border-border/40 bg-secondary/20 p-3", children: [_jsx("dt", { className: "text-xs font-mono uppercase tracking-wider text-cyan-300", children: f.label }), _jsx("dd", { className: "mt-1 text-xs leading-relaxed text-muted-foreground", children: f.desc })] }, f.label))) })] })), _jsxs("div", { className: "rounded-xl border border-border/40 bg-card/60 p-5 backdrop-blur-xl", children: [_jsx("h3", { className: "mb-3 text-sm font-display font-bold uppercase tracking-wider text-foreground", children: "Passi da seguire" }), _jsx("ul", { className: "space-y-2", children: chapter.steps.map((s, i) => {
                                            const checked = !!done[`${chapter.id}:${i}`];
                                            return (_jsx("li", { children: _jsxs("button", { onClick: () => toggleStep(i), className: `flex w-full items-start gap-3 rounded-lg border p-3 text-left text-sm transition-all ${checked
                                                        ? "border-emerald-500/40 bg-emerald-500/10 text-emerald-200"
                                                        : "border-border/40 bg-secondary/20 text-foreground hover:bg-secondary/40"}`, children: [checked ? _jsx(CheckCircle2, { size: 17, className: "mt-0.5 shrink-0 text-emerald-400" }) : _jsx(Circle, { size: 17, className: "mt-0.5 shrink-0 opacity-50" }), _jsxs("span", { children: [_jsxs("span", { className: "mr-2 font-mono text-xs opacity-60", children: [i + 1, "."] }), s] })] }) }, i));
                                        }) }), chapter.warnings?.length ? (_jsx("div", { className: "mt-4 space-y-2", children: chapter.warnings.map((w, i) => (_jsxs("div", { className: "flex items-start gap-2 rounded-lg border border-red-500/25 bg-red-500/10 p-3 text-xs leading-relaxed text-red-300", children: [_jsx(AlertTriangle, { size: 14, className: "mt-0.5 shrink-0" }), " ", _jsx("span", { children: w })] }, i))) })) : null, chapter.tip && (_jsxs("div", { className: "mt-4 rounded-lg border border-amber-500/25 bg-amber-500/10 p-3 text-xs leading-relaxed text-amber-300", children: ["\uD83D\uDCA1 ", chapter.tip] })), chapter.route && (_jsxs(Button, { className: "mt-4 gap-2", onClick: () => navigate(chapter.route), children: ["Provalo ora nel gestionale ", _jsx(ChevronRight, { size: 15 })] }))] }), chapter.faq && (_jsxs("div", { className: "rounded-xl border border-border/40 bg-card/60 p-5 backdrop-blur-xl", children: [_jsxs("h3", { className: "mb-3 flex items-center gap-2 text-sm font-display font-bold uppercase tracking-wider text-foreground", children: [_jsx(HelpCircle, { size: 15, className: "text-amber-400" }), " Domande frequenti"] }), _jsx("div", { className: "space-y-2", children: chapter.faq.map((f) => (_jsxs("details", { className: "rounded-lg border border-border/40 bg-secondary/20 p-3", children: [_jsx("summary", { className: "cursor-pointer text-sm text-foreground", children: f.q }), _jsx("p", { className: "mt-2 text-xs leading-relaxed text-muted-foreground", children: f.a })] }, f.q))) })] })), _jsxs("div", { className: "flex items-center justify-between pb-6", children: [_jsxs(Button, { variant: "ghost", disabled: index === 0, onClick: () => goTo(index - 1), className: "gap-1", children: [_jsx(ChevronLeft, { size: 16 }), " Precedente"] }), index < CHAPTERS.length - 1 ? (_jsxs(Button, { onClick: () => goTo(index + 1), className: "gap-1", children: ["Prossimo capitolo ", _jsx(ChevronRight, { size: 16 })] })) : (_jsx(Button, { variant: "outline", onClick: () => goTo(0), children: "Ricomincia da capo" }))] })] })] }), zoom && (_jsxs("div", { className: "fixed inset-0 z-[100] flex items-center justify-center bg-black/90 p-4", onClick: () => setZoom(false), children: [_jsx("button", { className: "absolute right-4 top-4 rounded-lg border border-border/50 bg-background/80 p-2", "aria-label": "Chiudi", children: _jsx(X, { size: 18 }) }), _jsx("img", { src: chapter.image, alt: `Schermata ingrandita ${chapter.title}`, className: "max-h-full max-w-full rounded-lg object-contain" })] }))] }));
}
