...
export default function MNAppCronologiaPage() {
    ...
    const filtered = allForms.filter((fir) => filter === "all" || statusMap[fir.status] === filter);
    const handleEdit = (fir) => { loadFromDatabase(fir); navigate(basePath); };
    const handleDelete = (fir) => { if (window.confirm(`Eliminare FIR ${fir.numero_fir || "senza numero"}?`))
