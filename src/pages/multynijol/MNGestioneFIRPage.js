...
export default function MNGestioneFIRPage() {
    ...
    const handleBulkImport = () => {
        ...
        const rows = unique.map(n => ({ fir_number: n, user_id: user.id, status: "available", societa_id: societaId }));
    ...
    const handleRequestFromRentri = async () => {
        ...
        try {
            ...
            if (realNumbers.length > 0) {
                const rows = realNumbers.map((n) => ({ fir_number: n, user_id: user.id, status: "available", societa_id: societaId }));
