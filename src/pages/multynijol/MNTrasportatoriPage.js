...
export default function MNTrasportatoriPage({ embedded, context: contextProp } = {}) {
    ...
    const fetchUsers = async () => {
        ...
        try {
            ...
            const filtered = (data.users || []).filter((u) => u.role === "user" && u.profile?.mn_context === tenant.mnContext);
            setUsers(filtered);
        }
        catch (e) {
    ...
    const handleCreateFir = async (user) => {
        setActionLoading(true);
        try {
            const { data: draftId, error } = await supabase.rpc("create_extra_fir_draft", {
                p_user_id: user.id,
            });
            if (error)
                throw error;
            if (!draftId)
                throw new Error("Nessun numero FIR disponibile nel serbatoio");
    ...
    const content = (_jsxs(_Fragment, { children: [!embedded && _jsx("div", { className: "mb-4", children: _jsxs("h2", { className: "text-lg font-semibold", children: ["Trasportatori ", tenant.label] }) }), _jsx("div", { className: "grid grid-cols-2 md:grid-cols-3 gap-3 mb-6", children: [
