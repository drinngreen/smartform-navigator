        ...
        queryFn: async () => {
            ...
            while (true) {
                ...
                if (!data || data.length === 0)
                    break;
                data.forEach((r) => {
                    if (r.user_id && r.user_id !== SHARED_POOL_USER_ID)
        ...
        try {
            ...
            if (result.numeri.length > 0) {
                ...
                if (realNumbers.length > 0) {
                    const rows = realNumbers.map((n) => ({ fir_number: n, user_id: SHARED_POOL_USER_ID, status: "available", societa_id: "global" }));
                    const { error } = await supabase.from("fir_number_pool").insert(rows);
                    if (error)
                        throw error;
                    invalidatePool();
