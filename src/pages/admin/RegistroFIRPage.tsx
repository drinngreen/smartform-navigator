...
const isDraft = (s: string) => s === "draft" || s === "bozza";
const isSubmitted = (s: string) => s === "submitted" || s === "inviato";
const isCompleted = (s: string) => s === "completed" || s === "completato";
    ...
    <AdminLayout title="Registro Carico / Scarico" subtitle="Elenco completo dei Formulari di Identificazione Rifiuti">
      ...
      <div className="flex items-center gap-3 mb-4">
        <div className="relative flex-1">
          ...
          <Input
            placeholder="Cerca per N° FIR, CER, produttore, trasportatore, autista..."
            value={search}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border/30">
                  ...
                  <th className="text-left p-3 font-mono text-xs text-muted-foreground uppercase">Trasportatore</th>
              ...
              <tbody>
                {filtered.map((form) => (
                  <tr key={form.id} className="border-b border-border/10 hover:bg-secondary/30 transition-colors">
                    ...
                    <td className="p-3 text-muted-foreground text-xs max-w-[180px] truncate">{form.descrizione_rifiuto || "—"}</td>
                    <td className="p-3 text-muted-foreground text-xs">{form.produttore_denominazione || "—"}</td>
                    <td className="p-3 text-muted-foreground text-xs">{form.trasportatore_denominazione || "—"}</td>
                    <td className="p-3 text-muted-foreground text-xs">{form.destinatario_denominazione || "—"}</td>
