...
import { MessageSquare, Send, Paperclip, ArrowLeft, Phone, User, Loader2 } from "lucide-react";
      ...
      <div className="flex h-[calc(100vh-180px)] rounded-2xl border border-border/30 bg-card/60 backdrop-blur-xl overflow-hidden">
        ...
        <div className={`w-80 border-r border-border/30 flex flex-col ${partnerId ? "hidden lg:flex" : "flex w-full lg:w-80"}`}>
          ...
          <div className="flex-1 overflow-y-auto">
            {loading && !partnerId ? (
              <div className="p-8 text-center"><Loader2 className="h-6 w-6 animate-spin text-cyan-400 mx-auto" /></div>
              ...
              conversations.map((conv) => (
                <button
                  key={conv.user_id}
                  onClick={() => navigate(`/admin/messaggi/${conv.user_id}`)}
                  className={`w-full text-left p-4 border-b border-border/10 hover:bg-white/5 transition-colors ${partnerId === conv.user_id ? "bg-cyan-500/10 border-l-2 border-l-cyan-400" : ""}`}
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-center">
                        ...
                        {conv.unread_count > 0 && (
                          <span className="ml-2 px-2 py-0.5 text-xs rounded-full bg-cyan-500 text-white font-bold">{conv.unread_count}</span>
                        )}
                      </div>
          ...
          {partnerId ? (
            <>
              ...
              <div className="p-4 border-b border-border/30 flex items-center gap-3">
                <button onClick={() => navigate("/admin/messaggi")} className="lg:hidden p-2 rounded-lg hover:bg-white/10">
                  ...
                  messages.map((msg) => {
                    ...
                    return (
                      <div key={msg.id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                        <div className={`max-w-[70%] px-4 py-2.5 rounded-2xl text-sm ${isMe ? "bg-cyan-600 text-white rounded-br-md" : "bg-white/10 text-foreground rounded-bl-md"}`}>
                          {msg.content && <p className="whitespace-pre-wrap">{msg.content}</p>}
                          {(msg as any).message_attachments?.map((att: any) => (
                            <AttachmentLink key={att.id} attachment={att} getUrl={getAttachmentUrl} />
                          ))}
                          <p className={`text-[10px] mt-1 ${isMe ? "text-cyan-200" : "text-white/40"}`}>
              ...
              <div className="p-4 border-t border-border/30">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className="flex-1 bg-white/10 text-foreground rounded-lg px-4 py-2.5 text-sm border border-border/30 placeholder:text-white/30 focus:outline-none focus:border-cyan-500"
                  />
                  <button
                    onClick={handleSend}
                    disabled={sending || (!text.trim() && files.length === 0)}
                    className="p-2.5 rounded-lg bg-cyan-600 text-white hover:bg-cyan-500 transition-colors disabled:opacity-40"
                  >
...
function AttachmentLink({ attachment, getUrl }: { attachment: any; getUrl: (path: string) => Promise<string | null> }) {
  ...
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" className="block text-xs text-cyan-300 underline mt-1">
