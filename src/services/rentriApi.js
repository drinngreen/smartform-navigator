...
function normalizeNumeroIscrizioneAlbo(raw) {
    const v = (raw || "")
        .trim()
        .toUpperCase()
        .replace(/\s+/g, "")
 ...
 */
function parseIndirizzo(raw, comuneIstatOverride) {
...
function lookupComuneId(city) {
    if (!city)
        return undefined;
    const normalized = city.trim().toLowerCase();
    if (COMUNE_ID_BY_NAME[normalized])
        return COMUNE_ID_BY_NAME[normalized];
    // Fuzzy fallback: try last token (e.g. "Santerno Ravenna" -> "ravenna")
    const tokens = normalized.split(/\s+/).filter(Boolean);
    if (tokens.length > 1) {
        const lastToken = tokens[tokens.length - 1];
        if (COMUNE_ID_BY_NAME[lastToken])
            return COMUNE_ID_BY_NAME[lastToken];
    }
    return undefined;
}
/**
...
function resolveStatoFisico(raw) {
...
function normalizeDestinatarioAttivita(raw) {
    const value = String(raw ?? "").trim().toUpperCase();
    if (/^[RD]\d{1,2}$/.test(value))
        return value;
    if (value === "R")
        return "R13";
    if (value === "D")
 ...
 */
function buildEmissionePayload(flat, societaId) {
    const str = (key) => flat[key] || "";
    const num = (key) => {
        const v = flat[key];
        if (typeof v === "number")
            return v;
    ...
    const formData = flat.form_data || {};
    const isUrbano = Boolean(formData.provenienza_urbano);
    const provenienza = isUrbano ? "U" : "S";
    const destinatarioAttivita = normalizeDestinatarioAttivita(formData.destinatario_operazione_R ||
        formData.destinatario_operazione_D ||
    ...
    const destComuneIstat = formData.destinatario_comune_istat || undefined;
    const prodComuneIstat = formData.produttore_comune_istat || undefined;
    ...
    const payload = {
        ...
        dati_partenza: {
            ...
            destinatario: {
                ...
                nazione_id: "IT",
                autorizzazione: {
                    tipo: str("destinatario_tipo_aut") || formData.destinatario_tipo || "AIA",
    ...
    if (hasIntermediario) {
        payload.dati_partenza.intermediario = {
...
export async function getRentriPdf(company, firId) {
    ...
    if (!res.ok) {
        const errMsg = normalized?.error || normalized?.details || `Errore server (${res.status})`;
        throw new Error(errMsg);
    }
    return normalized;
}
