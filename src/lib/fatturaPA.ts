...
export async function creaPrimaNotaDaFattura(f: any, tipo: "servizi" | "noleggio" = "servizi") {
  if (!f.tenant_id) throw new Error("tenant mancante");

  // Recupera conti ERP di default
  const [cliRes, ricRes, ivaRes, causaleRes] = await Promise.all([
    supabase.from("erp_piano_conti" as any).select("id").eq("tenant_id", f.tenant_id).eq("codice", "20.10.001").maybeSingle(),
    supabase.from("erp_piano_conti" as any).select("id").eq("tenant_id", f.tenant_id).eq("codice", tipo === "noleggio" ? "40.10.002" : "40.10.001").maybeSingle(),
    supabase.from("erp_piano_conti" as any).select("id").eq("tenant_id", f.tenant_id).eq("codice", "25.20.001").maybeSingle(),
    supabase.from("erp_causali_contabili" as any).select("id").eq("tenant_id", f.tenant_id).eq("codice", tipo === "noleggio" ? "FTV-NOL" : "FTV").maybeSingle(),
  ]);

  const conto_cliente_id = (cliRes.data as any)?.id;
  const conto_ricavi_id = (ricRes.data as any)?.id;
  const conto_iva_id = (ivaRes.data as any)?.id;
  const causale_id = (causaleRes.data as any)?.id;
