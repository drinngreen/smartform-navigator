...
const nx = (n) => n * W;
const ny = (n) => n * H;
function setFont(doc, size = 9) {
    doc.setFont("Courier", "normal");
    doc.setFontSize(size);
    doc.setTextColor(0, 0, 128); // #000080
}
function txt(doc, text, xNorm, yNorm, size = 9) {
    if (!text)
        return;
    setFont(doc, size);
    doc.text(text, nx(xNorm), ny(yNorm));
}
function txtR(doc, text, xNorm, yNorm, size = 9) {
    if (!text)
        return;
    setFont(doc, size);
...
// ── MAIN EXPORT ─────────────────────────────────────────
export async function generateFIRPdf(formData) {
    ...
    // ── SEZIONE 10: ALLEGATI ──
    if (d.allegatoMicroraccolta)
        mark(doc, 0.05, 0.640);
    if (d.allegatoIntermodale)
        mark(doc, 0.30, 0.640);
    // ── SEZIONE 12: ACCETTAZIONE DESTINATARIO ──
    txt(doc, d.dataOraArrivo, 0.05, 0.695);
    if (d.accettazione === "intero")
        mark(doc, 0.45, 0.695);
    ...
    if (d.inAttesaVerificaAnalitica)
        mark(doc, 0.55, 0.750);
