...
export async function generateFIRSummaryPdf(data, options = {}) {
    ...
    if (data.targaRimorchio)
        row("Rimorchio:", data.targaRimorchio);
    separator();
    // ── 4. Intermediario ──
    ...
    if (data.pesoRicevuto)
        row("Peso a Destino:", `${data.pesoRicevuto} kg`);
    ...
    if (data.dataFineTrasporto)
        row("Fine:", `${data.dataFineTrasporto} ${data.oraFineTrasporto}`);
    if (data.percorsoDiverso)
        row("Percorso:", data.percorsoDiverso);
    separator();
    // ── Footer ──
    y += 5;
