 ...
 */
export async function creaScritturaPrimaNota(params: CreaPrimaNotaParams) {
  ...
  const { data: numData } = await supabase.rpc("next_prima_nota_number" as any, {
  ...
  const righePayload = params.righe.map(r => ({
    prima_nota_id: (header as any).id,
    conto_id: r.conto_id,
    segno: r.segno,
    importo: r.importo,
...
export async function creaScritturaDaFatturaVendita(opts: {
  ...
  conto_cliente_id: string;
  conto_ricavi_id: string;
  conto_iva_id: string;
  conto_ritenuta_id?: string;
  causale_id?: string;
  created_by?: string;
}) {
  const righe: RigaPrimaNota[] = [
