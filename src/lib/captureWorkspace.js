...
function shrink(dataUrl, factor) {
...
export async function captureWorkspaceScreenshot() {
    try {
        ...
        const render = html2canvas(el, {
            ...
            ignoreElements: (node) => {
                ...
                if (tag === "iframe" || tag === "video" || tag === "audio" || tag === "script" || tag === "style" || tag === "noscript")
                    return true;
                if (e.dataset && e.dataset.darkLemon === "true")
                    return true;
                return false;
            },
        });
        ...
        if (dataUrl.length > MAX_DATAURL_BYTES)
            dataUrl = await shrink(dataUrl, 0.6);
        if (dataUrl.length > MAX_DATAURL_BYTES)
            dataUrl = await shrink(dataUrl, 0.5);
