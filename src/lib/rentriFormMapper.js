...
function normalizeFormData(formData, templateFields) {
    const result = {};
...
export function mapFormToRentriPayload(cliente, formData, options) {
    const cfg = TENANT_RENTRI[cliente];
    if (!cfg)
        throw new Error(`Configurazione RENTRI non trovata per: ${cliente}`);
    const data = normalizeFormData(formData, options?.templateFields);
    const str = (key) => String(data[key] || "").trim();
    const bool = (key) => data[key] === true || data[key] === "true" || str(key).toLowerCase() === "si";
    // Parse addresses
