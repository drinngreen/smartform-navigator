...
import { Upload, RefreshCw, Database, Package, CheckCircle, Clock, AlertTriangle, Zap, XCircle, ChevronLeft, ChevronRight, Search, UserPlus, Users, Printer, Plus } from "lucide-react";
...
export function DevGestioneFIRModule() {
    ...
    const [printFirNumber, setPrintFirNumber] = useState(null);
    ...
    const [blankPrintNumber, setBlankPrintNumber] = useState(null);
    ...
    const handleInlineAssign = async (poolId, firNumber, targetUserId) => {
        try {
            const { error } = await supabase.rpc("create_manual_fir_draft_for_tenant", {
    ...
    return (_jsxs("div", { className: "space-y-6", children: [_jsxs("div", { className: "grid grid-cols-2 lg:grid-cols-5 gap-4", children: [_jsx(StatCard, { icon: _jsx(Database, { className: "h-5 w-5" }), label: "Totale", value: stats?.total ?? 0, color: "text-primary", loading: statsLoading }), _jsx(StatCard, { icon: _jsx(CheckCircle, { className: "h-5 w-5" }), label: "Disponibili", value: stats?.disponibili ?? 0, color: "text-emerald-400", loading: statsLoading }), _jsx(StatCard, { icon: _jsx(Clock, { className: "h-5 w-5" }), label: "In Uso", value: stats?.inUso ?? 0, color: "text-cyan-400", loading: statsLoading }), _jsx(StatCard, { icon: _jsx(Package, { className: "h-5 w-5" }), label: "Consumati", value: stats?.usati ?? 0, color: "text-orange-400", loading: statsLoading }), _jsx(StatCard, { icon: _jsx(Printer, { className: "h-5 w-5" }), label: "Cartacei", value: stats?.cartacei ?? 0, color: "text-violet-400", loading: statsLoading })] }), _jsxs("div", { className: "rounded-2xl bg-card/60 border border-violet-500/30 p-6 space-y-4", children: [_jsxs("div", { className: "flex items-center gap-2 text-violet-400", children: [_jsx(Printer, { className: "h-5 w-5" }), _jsx("h3", { className: "font-display text-lg tracking-wider uppercase", children: "Stampa Formulario Vuoto" })] }), _jsx("p", { className: "text-xs text-muted-foreground", children: "Stampa un modulo completamente vuoto, da compilare a mano. Puoi assegnargli un numero FIR (opzionale)." }), _jsxs("div", { className: "flex flex-col gap-3 sm:flex-row", children: [_jsx("input", { value: blankFirNumber, onChange: (e) => setBlankFirNumber(e.target.value.toUpperCase()), onKeyDown: (e) => { if (e.key === "Enter") setBlankPrintNumber(blankFirNumber.trim()); }, placeholder: "NUMERO FIR (opzionale)", className: "min-w-0 flex-1 bg-background/80 border border-violet-500/40 rounded-xl px-4 py-3 text-foreground text-sm font-mono placeholder:text-muted-foreground/40 focus:outline-none focus:ring-1 focus:ring-violet-400" }), _jsxs("button", { onClick: () => setBlankPrintNumber(blankFirNumber.trim()), className: "px-6 py-3 rounded-xl bg-violet-600 text-white font-display text-sm tracking-wider hover:bg-violet-500 transition-colors flex items-center justify-center gap-2", children: [_jsx(Printer, { className: "h-4 w-4" }), " STAMPA VUOTO"] })] })] }), _jsxs("div", { className: "rounded-2xl bg-card/60 border border-emerald-500/30 p-6 space-y-4", children: [_jsxs("div", { className: "flex items-center gap-2 text-emerald-400", children: [_jsx(Plus, { className: "h-5 w-5" }), _jsx("h3", { className: "font-display text-lg tracking-wider uppercase", children: "Crea Formulario da Numero" })] }), _jsxs("div", { className: "flex flex-col gap-3 sm:flex-row", children: [_jsx("input", { value: manualFirNumber, onChange: (e) => setManualFirNumber(e.target.value.toUpperCase()), onKeyDown: (e) => { if (e.key === "Enter")
                            ...
                            setBlankFirNumber("");
                        } })), printFirNumber && (_jsx(DevStampaFIREditor, { firNumber: printFirNumber, open: !!printFirNumber, onClose: () => setPrintFirNumber(null), onPrinted: async () => {
                    // Mark as cartaceo
                    const { error } = await supabase
                        .from("fir_number_pool")
                        .update({ status: "cartaceo", consumed_at: new Date().toISOString() })
