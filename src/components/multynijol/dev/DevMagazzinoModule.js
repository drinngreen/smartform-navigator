...
export function DevMagazzinoModule() {
    ...
    const invalidateAll = () => {
        ["dev-mag-giacenze", "dev-mag-movimenti", "dev-cernite", "dev-giacenze", "dev-movimenti-multy", "dev-registro-movimenti"].forEach((k) => queryClient.invalidateQueries({ queryKey: [k] }));
    };
    // Recalculate
    ...
    const handleSaveCernita = async () => {
        ...
        if (!(inputKg > 0)) {
            toast.error("Quantità in ingresso non valida");
            return;
        }
        if (outputKg > inputKg) {
        ...
        if (inputKg > disponibile) {
            toast.error(`Giacenza insufficiente: disponibili ${disponibile.toLocaleString("it-IT")} kg`);
            return;
        }
        try {
            ...
            const { error } = await supabase.rpc("esegui_cernita_atomica", {
                p_tenant_id: MULTY_TENANT_ID,
