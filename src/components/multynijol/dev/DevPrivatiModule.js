...
export function DevPrivatiModule() {
    ...
    const [righeMateriali, setRigheMateriali] = useState([{ cer: "", kg: "" }]);
    const [openCerRow, setOpenCerRow] = useState(null);
    const [mostraTuttiCer, setMostraTuttiCer] = useState(false);
    const cerRowRefs = useRef(new Map());
    ...
    const [editDateConfId, setEditDateConfId] = useState(null);
    const [editDateValue, setEditDateValue] = useState();
    const [editVeicoloConfId, setEditVeicoloConfId] = useState(null);
    const [editVeicoloForm, setEditVeicoloForm] = useState({ targa_automezzo: "", modello_automezzo: "" });
    const handleUpdateConfVeicolo = async (confId) => {
    ...
    const handleUpdateConfDate = async (confId, newDate) => {
        const iso = format(newDate, "yyyy-MM-dd");
        const { error } = await supabase.from("privati_conferimenti").update({ data: iso }).eq("id", confId);
        if (error) {
            toast.error(error.message);
            return;
        }
        const { error: ricevutaError } = await supabase
            .from("ricevute_privati")
            .update({ data_emissione: iso })
            .eq("conferimento_id", confId);
        if (ricevutaError) {
            toast.error(ricevutaError.message);
            return;
        }
    ...
    const handleDeleteConferimento = async (confId) => {
        if (!window.confirm("Eliminare questo conferimento? Le giacenze verranno stornate automaticamente."))
            return;
    ...
    const cerOptions = (q) => searchCerList(q);
    ...
    const handleSaveConferimento = async () => {
        ...
        for (const r of righeMateriali) {
            ...
            if (!rawCer && !r.kg)
                continue;
            if (!rawCer) {
                toast.error("Ogni riga deve avere un codice CER");
                return;
            }
            if (!Number.isFinite(kg) || kg <= 0) {
                toast.error(`Peso non valido per il CER ${rawCer}`);
                return;
            }
        ...
        if (!righe.length) {
            toast.error("Inserisci almeno un materiale (CER + kg)");
            return;
        }
        ...
        logAgentActivity("Creazione conferimento privato", "ok", `${righe.length} materiali (${righe.map((r) => r.cer).join(", ")}) — data ${dataRegistrazione}`);
    ...
    return (_jsxs("div", { className: "space-y-4", children: [_jsx(PrivatiLimitiWidget, { tenantId: MULTY_TENANT_ID }), _jsxs(Card, { className: "bg-red-950/30 border-red-500/30", children: [_jsx(CardHeader, { children: _jsxs(CardTitle, { className: "text-red-400 flex items-center gap-2", children: [_jsx(ShieldAlert, { className: "h-5 w-5" }), " Limite Annuo per Privato: ", LIMITE_ANNUO_GLOBALE_KG, " kg"] }) }), _jsx(CardContent, { children: _jsxs("p", { className: "text-sm text-muted-foreground", children: ["Ogni privato pu\u00F2 conferire al massimo ", _jsxs("strong", { className: "text-red-400", children: [LIMITE_ANNUO_GLOBALE_KG, " kg"] }), " totali all'anno (tutti i CER sommati). Il sistema blocca automaticamente i conferimenti oltre soglia."] }) })] }), _jsxs("div", { className: "flex gap-2 flex-wrap", children: [_jsxs(Button, { onClick: () => { setEditPrivatoId(null); setPrivatoForm({ ...EMPTY_PRIVATO_FORM }); setScadenzaDate(undefined); setShowNewPrivato(true); }, className: "gap-2 bg-emerald-600 hover:bg-emerald-700", children: [_jsx(Plus, { className: "h-4 w-4" }), " Nuovo Privato"] }), _jsxs(Button, { onClick: handleOpenConferimento, variant: "outline", className: "gap-2 border-emerald-500/30 text-emerald-400", children: [_jsx(Scale, { className: "h-4 w-4" }), " Nuovo Conferimento"] }), _jsxs(Button, { onClick: () => {
                            ...
                            setShowNewRicevuta(true);
                                    ...
                                    return (_jsxs("div", { onClick: () => setSelectedPrivatoId(p.id), className: `p-3 rounded cursor-pointer mb-1 border transition-all ${selectedPrivatoId === p.id ? "bg-emerald-500/10 border-emerald-500/30" : "bg-card/30 border-border/10 hover:bg-white/5"}`, children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("div", { children: [_jsxs("span", { className: "font-medium", children: [p.cognome, " ", p.nome] }), _jsx("span", { className: "ml-2 text-xs text-muted-foreground font-mono", children: p.codice_fiscale })] }), _jsxs("div", { className: "flex items-center gap-2 shrink-0", children: [hasWarning && _jsx(AlertTriangle, { className: "h-4 w-4 text-amber-400" }), _jsxs("button", { className: "h-7 px-2 inline-flex items-center gap-1 rounded-md border border-emerald-500/40 hover:bg-emerald-500/20 text-emerald-400 text-xs font-medium", onClick: (e) => { e.stopPropagation(); openEditPrivato(p); }, title: "Modifica privato", children: [_jsx(Edit2, { className: "h-3.5 w-3.5" }), "Modifica"] })] })] }), _jsxs("div", { className: "text-xs text-muted-foreground mt-1", children: [p.comune_residenza || "-", p.targa_automezzo ? ` · ${p.targa_automezzo}` : ""] })] }, p.id));
                                                ...
                                                return (_jsxs("div", { className: "space-y-3", children: [_jsxs("div", { className: "space-y-1", children: [_jsxs("div", { className: "flex justify-between text-sm", children: [_jsx("span", { className: "font-medium", children: "Totale Annuo" }), _jsxs("span", { className: isOver ? "text-red-400 font-bold" : isWarn ? "text-amber-400" : "", children: [totalKg.toLocaleString("it-IT"), " kg / ", LIMITE_ANNUO_GLOBALE_KG, " kg"] })] }), _jsx("div", { className: "h-2 bg-card/60 rounded-full overflow-hidden", children: _jsx("div", { className: `h-full rounded-full transition-all ${isOver ? "bg-red-500" : isWarn ? "bg-amber-500" : "bg-emerald-500"}`, style: { width: `${Math.min(pct, 100)}%` } }) })] }), Object.keys(selectedUsage).length === 0 ? (_jsx("p", { className: "text-muted-foreground text-sm", children: "Nessun conferimento quest'anno" })) : (_jsx("div", { className: "space-y-1", children: Object.entries(selectedUsage).map(([cer, kg]) => (_jsxs("div", { className: "flex justify-between text-sm", children: [_jsx("span", { className: "font-mono", children: cer }), _jsxs("span", { children: [kg.toLocaleString("it-IT"), " kg"] })] }, cer))) })), isOver && (_jsxs("div", { className: "flex items-center gap-1 text-red-400 text-xs", children: [_jsx(AlertTriangle, { className: "h-3 w-3" }), " LIMITE SUPERATO \u2014 Operazione bloccata"] }))] }));
                                            })() })] }), _jsxs(Card, { className: "bg-card/60 border-border/30", children: [_jsx(CardHeader, { children: _jsxs(CardTitle, { className: "text-sm flex items-center gap-2", children: [_jsx(Scale, { className: "h-4 w-4" }), " Conferimenti (", conferimentiPrivato?.length ?? 0, ")"] }) }), _jsx(CardContent, { className: "max-h-80 overflow-y-auto", children: !conferimentiPrivato?.length ? (_jsx("p", { className: "text-muted-foreground text-xs", children: "Nessun conferimento registrato" })) : (_jsx("div", { className: "space-y-1", children: conferimentiPrivato.map((c) => (_jsxs("div", { className: "flex items-center gap-2 text-sm p-2 rounded bg-card/30 border border-border/20", children: [_jsxs("div", { className: "flex-1 min-w-0", children: [_jsxs("div", { className: "flex items-center gap-2 flex-wrap", children: [c.numero_progressivo != null && (_jsxs("span", { className: "font-mono text-xs px-1.5 py-0.5 rounded bg-emerald-500/15 text-emerald-300 border border-emerald-500/30", children: ["DBT #", c.numero_progressivo, "/", c.anno_dbt ?? new Date(c.data).getFullYear()] })), _jsx("span", { className: "font-mono text-xs", children: c.cer }), _jsxs("span", { className: "font-medium", children: [Number(c.kg_pesati).toLocaleString("it-IT"), " kg"] }), c.importo_pagato != null && (_jsxs("span", { className: "text-xs text-emerald-400", children: ["\u20AC ", Number(c.importo_pagato).toFixed(2)] })), c.metodo_pag && (_jsx("span", { className: cn("text-[10px] font-mono uppercase px-1.5 py-0.5 rounded border", c.metodo_pag === "contanti"
                                                                                ? "bg-amber-500/10 text-amber-300 border-amber-500/30"
                                                                                : "bg-sky-500/10 text-sky-300 border-sky-500/30"), children: c.metodo_pag === "contanti" ? "Contanti" : "Tracciabile" }))] }), _jsxs("div", { className: "text-xs text-muted-foreground", children: ["Data registrazione: ", toLocalDateLabel(c.data), c.targa_automezzo ? ` · ${c.targa_automezzo}` : "", c.modello_automezzo ? ` · ${c.modello_automezzo}` : ""] })] }), _jsxs(Popover, { open: editVeicoloConfId === c.id, onOpenChange: (o) => { if (!o)
                                                                setEditVeicoloConfId(null); }, children: [_jsx(PopoverTrigger, { asChild: true, children: _jsx(Button, { variant: "outline", size: "sm", className: "h-7 border-sky-500/30 text-sky-400 hover:bg-sky-500/10", onClick: () => {
                                                                            setEditVeicoloConfId(c.id);
                                                                            setEditVeicoloForm({ targa_automezzo: c.targa_automezzo || "", modello_automezzo: c.modello_automezzo || "" });
                                                                        }, title: "Modifica targa / veicolo", children: _jsx(Truck, { className: "h-3 w-3" }) }) }), _jsxs(PopoverContent, { className: "w-64 p-3 space-y-2", align: "end", children: [_jsxs("div", { children: [_jsx(Label, { className: "text-xs", children: "Targa Automezzo" }), _jsx(Input, { className: "font-mono", value: editVeicoloForm.targa_automezzo, onChange: (e) => setEditVeicoloForm(f => ({ ...f, targa_automezzo: e.target.value.toUpperCase() })) })] }), _jsxs("div", { children: [_jsx(Label, { className: "text-xs", children: "Modello Veicolo" }), _jsx(Input, { value: editVeicoloForm.modello_automezzo, onChange: (e) => setEditVeicoloForm(f => ({ ...f, modello_automezzo: e.target.value })) })] }), _jsx(Button, { size: "sm", className: "w-full", onClick: () => handleUpdateConfVeicolo(c.id), children: "Salva" })] })] }), _jsxs(Popover, { open: editDateConfId === c.id, onOpenChange: (o) => { if (!o)
                                                                setEditDateConfId(null); }, children: [_jsx(PopoverTrigger, { asChild: true, children: _jsx(Button, { variant: "outline", size: "sm", className: "h-7 border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/10", onClick: () => { setEditDateConfId(c.id); setEditDateValue(c.data ? new Date(c.data) : undefined); }, title: "Modifica data", children: _jsx(CalendarIcon, { className: "h-3 w-3" }) }) }), _jsx(PopoverContent, { className: "w-auto p-0", align: "end", children: _jsx(Calendar, { mode: "single", selected: editDateValue, onSelect: (d) => { if (d)
                                                                            handleUpdateConfDate(c.id, d); }, initialFocus: true, className: cn("p-3 pointer-events-auto") }) })] }), _jsx(Button, { variant: "outline", size: "sm", className: "h-7 border-destructive/30 text-destructive hover:bg-destructive/10", onClick: () => handleDeleteConferimento(c.id), title: "Elimina conferimento", children: _jsx(Trash2, { className: "h-3 w-3" }) })] }, c.id))) })) })] }), _jsxs(Card, { className: "bg-card/60 border-border/30", children: [_jsx(CardHeader, { children: _jsxs(CardTitle, { className: "text-sm flex items-center gap-2", children: [_jsx(FileText, { className: "h-4 w-4" }), " Documenti Scansionati"] }) }), _jsxs(CardContent, { children: [_jsx("input", { type: "file", ref: fileInputRef, className: "hidden", accept: ".pdf,.jpg,.jpeg,.png", onChange: (e) => { const f = e.target.files?.[0]; if (f)
                                                        uploadDoc.mutate(f); } }), _jsxs(Button, { variant: "outline", size: "sm", onClick: () => fileInputRef.current?.click(), disabled: uploadDoc.isPending, className: "gap-2 mb-3 border-emerald-500/30 text-emerald-400", children: [_jsx(Upload, { className: "h-4 w-4" }), " Carica Documento"] }), documenti?.length ? (_jsx("div", { className: "space-y-1", children: documenti.map((d) => (_jsxs("div", { className: "flex items-center gap-2 text-sm p-2 rounded bg-card/30", children: [_jsx(FileText, { className: "h-4 w-4 text-muted-foreground" }), _jsx("span", { children: d.nome_file }), _jsx("span", { className: "text-xs text-muted-foreground ml-auto", children: new Date(d.created_at).toLocaleDateString("it-IT") })] }, d.id))) })) : (_jsx("p", { className: "text-muted-foreground text-xs", children: "Nessun documento caricato" }))] })] }), _jsxs(Card, { className: "bg-card/60 border-border/30", children: [_jsx(CardHeader, { children: _jsxs("div", { className: "flex items-center justify-between gap-2", children: [_jsxs(CardTitle, { className: "text-sm flex items-center gap-2", children: [_jsx(Receipt, { className: "h-4 w-4" }), " Ricevute (", ricevute?.length ?? 0, ")"] }), _jsxs("div", { className: "flex gap-1", children: [_jsxs(Button, { variant: "outline", size: "sm", onClick: handlePrintRicevute, className: "gap-1 border-emerald-500/30 text-emerald-400 h-7 text-xs", children: [_jsx(Printer, { className: "h-3 w-3" }), " Stampa"] }), _jsxs(Button, { variant: "outline", size: "sm", onClick: () => {
                                                                    ...
                                                                    const cols = [
                                                                        ...
                                                                        { header: "Data", key: "data_emissione", width: 14, format: (v) => v ? new Date(v).toLocaleDateString("it-IT") : "-" },
