...
import { Search, RefreshCw, Loader2, Plus, ArrowDownToLine, ArrowUpFromLine, FileSpreadsheet, Printer, AlertTriangle, } from "lucide-react";
...
export function DevRegistroCaricoScaricoModule() {
    ...
    const stats = useMemo(() => ({
        ...
        carico: movimenti.filter((m) => m.tipo_movimento === "CARICO").length,
        scarico: movimenti.filter((m) => m.tipo_movimento === "SCARICO").length,
        totalKg: movimenti.reduce((s, m) => s + (m.quantita_kg || 0), 0),
    }), [movimenti]);
    // CER selector filtering
    const filteredCER = useMemo(() => {
        if (!cerSearch.trim())
            return CER_PREFERITI.slice(0, 20);
        const q = cerSearch.toLowerCase();
        return CER_PREFERITI.filter((c) => c.codice.toLowerCase().includes(q) || c.descrizione.toLowerCase().includes(q));
    }, [cerSearch]);
    const selectCER = (cer) => {
        setForm((f) => ({
            ...f,
    ...
    const handleSave = async () => {
        if (!form.cer.trim()) {
            toast.error("Seleziona un codice CER");
            return;
        }
        if (!form.quantita_kg || parseFloat(form.quantita_kg) <= 0) {
            toast.error("Inserisci una quantità valida");
            return;
        }
        if (!impianto?.id) {
            toast.error("Nessun impianto trovato per questo tenant");
            return;
        }
        setSaving(true);
        try {
            ...
            ["dev-registro-movimenti", "dev-movimenti-multy", "dev-mag-movimenti", "dev-mag-giacenze", "dev-giacenze"].forEach((k) => queryClient.invalidateQueries({ queryKey: [k] }));
        }
        catch (e) {
    ...
    return (_jsxs("div", { className: "space-y-4", children: [_jsx("div", { className: "grid grid-cols-2 md:grid-cols-4 gap-3", children: [
                    ...
                    { label: "Kg Totali", value: stats.totalKg.toLocaleString("it-IT"), icon: "⚖️" },
                ].map((s) => (_jsx(Card, { className: "bg-card/60 border-emerald-500/30", children: _jsxs(CardContent, { className: "p-4", children: [_jsxs("div", { className: "flex items-center gap-2 mb-1", children: [_jsx("span", { children: s.icon }), _jsx("span", { className: "text-xs text-muted-foreground uppercase", children: s.label })] }), _jsx("span", { className: "text-2xl font-bold text-foreground", children: s.value })] }) }, s.label))) }), _jsxs("div", { className: "flex items-center gap-3 flex-wrap", children: [_jsxs("div", { className: "relative flex-1 min-w-[200px]", children: [_jsx(Search, { className: "absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" }), _jsx(Input, { placeholder: "Cerca CER, produttore, FIR...", value: search, onChange: (e) => setSearch(e.target.value), className: "pl-10 bg-card/60 border-border/30" })] }), _jsxs(Select, { value: filterTipo, onValueChange: setFilterTipo, children: [_jsx(SelectTrigger, { className: "w-[140px] bg-card/60 border-border/30", children: _jsx(SelectValue, {}) }), _jsxs(SelectContent, { children: [_jsx(SelectItem, { value: "all", children: "Tutti" }), _jsx(SelectItem, { value: "CARICO", children: "Carico" }), _jsx(SelectItem, { value: "SCARICO", children: "Scarico" })] })] }), _jsx(Button, { variant: "outline", size: "icon", onClick: () => refetch(), disabled: isLoading, className: "border-emerald-500/30 text-emerald-400", children: _jsx(RefreshCw, { className: `h-4 w-4 ${isLoading ? "animate-spin" : ""}` }) }), _jsxs(Button, { onClick: () => setAddOpen(true), className: "gap-2 bg-emerald-600 hover:bg-emerald-700", children: [_jsx(Plus, { className: "h-4 w-4" }), " Nuovo Movimento"] }), _jsxs(Button, { variant: "outline", size: "sm", onClick: () => {
                            if (!filtered.length)
                                return toast.error("Nessun movimento");
                            const cols = [
                                { header: "Data", key: "data_movimento", width: 12, format: (v) => new Date(v).toLocaleDateString("it-IT") },
                        ...
                        }, className: "gap-1 border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/10", children: [_jsx(Printer, { className: "h-3 w-3" }), " PDF"] })] }), isLoading ? (_jsx("div", { className: "flex items-center justify-center py-12", children: _jsx(Loader2, { className: "h-8 w-8 animate-spin text-emerald-400" }) })) : (_jsx(Card, { className: "bg-card/60 border-border/30", children: _jsx(CardContent, { className: "p-0", children: _jsx("div", { className: "overflow-x-auto", children: _jsxs("table", { className: "w-full text-sm", children: [_jsx("thead", { children: _jsxs("tr", { className: "border-b border-border/30 text-muted-foreground", children: [_jsx("th", { className: "text-left p-3 text-xs uppercase", children: "Data" }), _jsx("th", { className: "text-left p-3 text-xs uppercase", children: "Tipo" }), _jsx("th", { className: "text-left p-3 text-xs uppercase", children: "Ruolo" }), _jsx("th", { className: "text-left p-3 text-xs uppercase", children: "CER" }), _jsx("th", { className: "text-left p-3 text-xs uppercase", children: "Descrizione" }), _jsx("th", { className: "text-right p-3 text-xs uppercase", children: "Kg" }), _jsx("th", { className: "text-left p-3 text-xs uppercase", children: "Produttore" }), _jsx("th", { className: "text-left p-3 text-xs uppercase", children: "Trasportatore" }), _jsx("th", { className: "text-left p-3 text-xs uppercase", children: "N\u00B0 FIR" }), _jsx("th", { className: "text-left p-3 text-xs uppercase", children: "Esito" })] }) }), _jsxs("tbody", { children: [filtered.map((m) => (_jsxs("tr", { className: "border-b border-border/10 hover:bg-muted/5", children: [_jsx("td", { className: "p-3 text-xs text-muted-foreground whitespace-nowrap", children: new Date(m.data_movimento).toLocaleDateString("it-IT") }), _jsx("td", { className: "p-3", children: _jsx(Badge, { variant: "outline", className: m.tipo_movimento === "CARICO"
                                                            ? "border-emerald-500/40 text-emerald-400"
                                                            : "border-blue-500/40 text-blue-400", children: m.tipo_movimento === "CARICO" ? (_jsxs(_Fragment, { children: [_jsx(ArrowDownToLine, { className: "h-3 w-3 mr-1" }), "Carico"] })) : (_jsxs(_Fragment, { children: [_jsx(ArrowUpFromLine, { className: "h-3 w-3 mr-1" }), "Scarico"] })) }) }), _jsx("td", { className: "p-3 text-xs text-muted-foreground", children: m.ruolo_impianto }), _jsxs("td", { className: "p-3 font-mono text-foreground", children: [m.cer, CER_PREFERITI.find(c => c.codice === m.cer)?.pericoloso && (_jsx(AlertTriangle, { className: "inline h-3 w-3 ml-1 text-amber-400" }))] }), _jsx("td", { className: "p-3 text-foreground max-w-[200px] truncate", title: m.descrizione_rifiuto, children: m.descrizione_rifiuto || "—" }), _jsx("td", { className: "p-3 text-right font-mono text-foreground", children: m.quantita_kg?.toLocaleString("it-IT") }), _jsx("td", { className: "p-3 text-muted-foreground text-xs", children: m.produttore_denominazione || "—" }), _jsx("td", { className: "p-3 text-muted-foreground text-xs", children: m.trasportatore_denominazione || "—" }), _jsx("td", { className: "p-3 font-mono text-xs text-emerald-400", children: m.numero_fir || "—" }), _jsx("td", { className: "p-3", children: m.esito_accettazione && (_jsx(Badge, { variant: m.esito_accettazione === "accettato" ? "default" : "destructive", className: "text-[10px]", children: m.esito_accettazione })) })] }, m.id))), filtered.length === 0 && (_jsx("tr", { children: _jsx("td", { colSpan: 10, className: "p-8 text-center text-muted-foreground", children: "Nessun movimento trovato" }) }))] })] }) }) }) })), _jsx(Dialog, { open: addOpen, onOpenChange: setAddOpen, children: _jsxs(DialogContent, { className: "max-w-2xl max-h-[85vh] overflow-y-auto", children: [_jsx(DialogHeader, { children: _jsx(DialogTitle, { children: "Nuovo Movimento \u2014 Registro Carico/Scarico" }) }), _jsxs("div", { className: "grid grid-cols-2 gap-4", children: [_jsxs("div", { children: [_jsx(Label, { children: "Tipo Movimento" }), _jsxs(Select, { value: form.tipo_movimento, onValueChange: (v) => setForm((f) => ({ ...f, tipo_movimento: v })), children: [_jsx(SelectTrigger, { children: _jsx(SelectValue, {}) }), _jsxs(SelectContent, { children: [_jsx(SelectItem, { value: "CARICO", children: "\uD83D\uDCE5 Carico" }), _jsx(SelectItem, { value: "SCARICO", children: "\uD83D\uDCE4 Scarico" })] })] })] }), _jsxs("div", { children: [_jsx(Label, { children: "Ruolo Impianto" }), _jsxs(Select, { value: form.ruolo_impianto, onValueChange: (v) => setForm((f) => ({ ...f, ruolo_impianto: v })), children: [_jsx(SelectTrigger, { children: _jsx(SelectValue, {}) }), _jsxs(SelectContent, { children: [_jsx(SelectItem, { value: "PRODUTTORE", children: "Produttore" }), _jsx(SelectItem, { value: "DESTINATARIO", children: "Destinatario" }), _jsx(SelectItem, { value: "TRATTAMENTO_INTERNO", children: "Trattamento Interno" })] })] })] }), _jsxs("div", { className: "col-span-2", children: [_jsx(Label, { children: "Codice CER (da preferiti)" }), _jsxs("div", { className: "relative", children: [_jsx(Input, { placeholder: "Cerca CER preferito...", value: form.cer ? `${form.cer} — ${form.descrizione_rifiuto}` : cerSearch, onChange: (e) => {
                                                        setCerSearch(e.target.value);
                                                        if (form.cer)
                                                            setForm((f) => ({ ...f, cer: "", descrizione_rifiuto: "" }));
                                                    }, className: "bg-card/60 border-border/30" }), cerSearch.trim() && !form.cer && (_jsxs("div", { className: "absolute z-50 top-full left-0 right-0 mt-1 bg-popover border border-border rounded-lg shadow-lg max-h-48 overflow-y-auto", children: [filteredCER.map((c) => (_jsxs("button", { onClick: () => selectCER(c), className: "w-full text-left px-3 py-2 hover:bg-muted/30 flex items-center gap-2 text-sm", children: [_jsx("span", { className: "font-mono font-semibold text-foreground", children: c.codice }), c.pericoloso && _jsx(AlertTriangle, { className: "h-3 w-3 text-amber-400 shrink-0" }), _jsx("span", { className: "text-muted-foreground truncate", children: c.descrizione })] }, c.codice))), filteredCER.length === 0 && (_jsx("div", { className: "px-3 py-2 text-muted-foreground text-sm", children: "Nessun CER trovato" }))] }))] })] }), _jsxs("div", { children: [_jsx(Label, { children: "Quantit\u00E0 (Kg) *" }), _jsx(Input, { type: "number", value: form.quantita_kg, onChange: (e) => setForm((f) => ({ ...f, quantita_kg: e.target.value })), className: "bg-card/60 border-border/30" })] }), _jsxs("div", { children: [_jsx(Label, { children: "Quantit\u00E0 Presunta (Kg)" }), _jsx(Input, { type: "number", value: form.quantita_presunta, onChange: (e) => setForm((f) => ({ ...f, quantita_presunta: e.target.value })), className: "bg-card/60 border-border/30" })] }), _jsxs("div", { children: [_jsx(Label, { children: "Data Movimento" }), _jsx(Input, { type: "date", value: form.data_movimento, onChange: (e) => setForm((f) => ({ ...f, data_movimento: e.target.value })), className: "bg-card/60 border-border/30" })] }), _jsxs("div", { children: [_jsx(Label, { children: "Numero FIR" }), _jsx(Input, { value: form.numero_fir, onChange: (e) => setForm((f) => ({ ...f, numero_fir: e.target.value })), className: "bg-card/60 border-border/30" })] }), _jsxs("div", { children: [_jsx(Label, { children: "Produttore" }), _jsx(Input, { value: form.produttore_denominazione, onChange: (e) => setForm((f) => ({ ...f, produttore_denominazione: e.target.value })), className: "bg-card/60 border-border/30" })] }), _jsxs("div", { children: [_jsx(Label, { children: "Trasportatore" }), _jsx(Input, { value: form.trasportatore_denominazione, onChange: (e) => setForm((f) => ({ ...f, trasportatore_denominazione: e.target.value })), className: "bg-card/60 border-border/30" })] }), _jsxs("div", { children: [_jsx(Label, { children: "Destinatario" }), _jsx(Input, { value: form.destinatario_denominazione, onChange: (e) => setForm((f) => ({ ...f, destinatario_denominazione: e.target.value })), className: "bg-card/60 border-border/30" })] }), _jsxs("div", { children: [_jsx(Label, { children: "Esito Accettazione" }), _jsxs(Select, { value: form.esito_accettazione, onValueChange: (v) => setForm((f) => ({ ...f, esito_accettazione: v })), children: [_jsx(SelectTrigger, { children: _jsx(SelectValue, {}) }), _jsxs(SelectContent, { children: [_jsx(SelectItem, { value: "accettato", children: "Accettato" }), _jsx(SelectItem, { value: "respinto", children: "Respinto" }), _jsx(SelectItem, { value: "parziale", children: "Parziale" })] })] })] }), _jsxs("div", { className: "col-span-2", children: [_jsx(Label, { children: "Note" }), _jsx(Textarea, { value: form.note, onChange: (e) => setForm((f) => ({ ...f, note: e.target.value })), className: "bg-card/60 border-border/30" })] })] }), _jsxs(DialogFooter, { children: [_jsx(Button, { variant: "outline", onClick: () => setAddOpen(false), children: "Annulla" }), _jsxs(Button, { onClick: handleSave, disabled: saving, className: "bg-emerald-600 hover:bg-emerald-700", children: [saving && _jsx(Loader2, { className: "h-4 w-4 animate-spin mr-2" }), "Registra Movimento"] })] })] }) })] }));
}
