...
export function MNAdminLayout({ children, title, subtitle }) {
    ...
    const accentColor = useMemo(() => {
        ...
        if (location.pathname.includes("/mn/admin/dev-multyproget"))
            return "34, 197, 94";
        if (location.pathname.includes("/mn/admin/niyol"))
            return "6, 182, 212";
        if (location.pathname.includes("/mn/admin/multyproget"))
            return "249, 115, 22";
        return routeColors["/mn/admin"];
    }, [location.pathname]);
    return (_jsx("div", { "data-admin-layout": true, className: "flex h-screen bg-background overflow-hidden relative", children: _jsxs("div", { className: `flex flex-col flex-1 overflow-hidden relative transition-all duration-300 ${sidePanel ? "mr-[max(20vw,280px)]" : ""}`, children: [!isDashboard && (_jsx("div", { className: "absolute inset-0 pointer-events-none transition-all duration-700 ease-in-out", style: {
                        background: `
