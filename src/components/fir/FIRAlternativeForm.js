...
function isNumeroFirFieldName(fieldName) {
...
function toStatoFisicoLabel(value) {
    if (typeof value !== "string")
        return null;
    const v = value.trim();
    if (!v)
        return null;
    if (STATO_FISICO_LABEL_MAP[v])
        return STATO_FISICO_LABEL_MAP[v];
    const lower = v.toLowerCase();
    if (ALLOWED_STATO_FISICO_LABELS.has(lower))
        return lower;
    return null;
}
...
function getDraftValueForField(field, draft, formData) {
    ...
    if (field.type === "date") {
        ...
        if (hasTokens(field.name, ["data", "arrivo", "destinatario"]) && !isSecondDestField)
            return getFormDataValue(formData, "data_accettazione") || draft.data_arrivo;
    ...
    if (field.type === "time") {
        ...
        if (hasTokens(field.name, ["ora", "arrivo", "destinatario"]) && !isSecondDestField)
            return getFormDataValue(formData, "ora_accettazione") || draft.data_arrivo;
...
export function FIRAlternativeForm({ presetNumeroFir, firFormId, assignedUserId, impiantoId, draftData, ocrEntries, printOnly, blankPrint, disableRentriActions, registryMovementType, onSaved, onPrinted } = {}) {
    ...
    useFormBridgeFields(() => fields.map((field) => {
        ...
        return {
            ...
            getValue: () => {
                if (isNumeroFirFieldName(field.name))
                    return canonicalNumeroFir;
    ...
    useEffect(() => {
        if (!ocrEntries?.length || !fields.length)
            return;
        setSuppressProducerPreset(true);
        const normalize = (value) => normalizeFieldName(value);
        const nextValues = {};
    ...
    useEffect(() => {
        ...
        if (ocrEntries?.length)
            return;
        if (suppressProducerPreset)
            return;
        if (tenantContext !== "multyproget" && tenantContext !== "niyol" && tenantContext !== "global")
            return;
        // Only auto-fill when the producer denomination is currently empty
        if (!produttoreDenomField)
            return;
    ...
    const isNumeroFirField = useCallback((field) => {
        if (!field)
            return false;
    ...
    const handleSaveDraft = async (mode = "draft") => {
        const targetId = firFormId || activeDraftId;
        if (!targetId) {
            toast.error("Nessuna bozza attiva");
            return;
        }
        const formatErr = (err) => {
            if (!err)
        ...
        try {
            ...
            const existingNumero = existing.numero_fir || null;
            const numeroFir = existingNumero || presetNumeroFir || activeDraftNumero || null;
            const tenantId = existing.tenant_id || TENANT_ID_MAP[tenantContext] || TENANT_ID_MAP.global;
            ...
            if (existingNumero) {
                mergedFormData.numero_fir = existingNumero;
                mergedFormData.numero_formulario = existingNumero;
            }
            const updates = { form_data: mergedFormData };
            if (desc)
                updates.descrizione_rifiuto = desc;
            if (eer)
                updates.codice_eer = eer;
            if (qta !== null)
                updates.quantita = qta;
            if (um)
                updates.unita_misura = um;
            const statoFisicoLabel = toStatoFisicoLabel(statoFisico);
            if (statoFisicoLabel)
                updates.stato_fisico = statoFisicoLabel;
            if (prodDen)
                updates.produttore_denominazione = prodDen;
            if (destDen)
                updates.destinatario_denominazione = destDen;
            if (trasDen)
                updates.trasportatore_denominazione = trasDen;
            if (prodCf)
                updates.produttore_codice_fiscale = prodCf;
            if (destCf)
                updates.destinatario_codice_fiscale = destCf;
            if (trasCf)
                updates.trasportatore_codice_fiscale = trasCf;
            if (dataEmissione)
                mergedFormData.data_emissione = dataEmissione;
            if (qta !== null) {
                mergedFormData.quantita_origine = qta;
                mergedFormData.quantita_partenza = qta;
            }
            if (qtaDestino !== null) {
                mergedFormData.quantita_destino = qtaDestino;
                mergedFormData.peso_ricevuto = qtaDestino;
            }
    ...
    return (_jsxs("div", { className: "space-y-3", children: [_jsxs("div", { className: "flex items-center gap-2 px-3 py-2 rounded-lg bg-amber-500/10 border border-amber-500/30", children: [_jsx(Zap, { className: "h-4 w-4 text-amber-400" }), _jsxs("div", { className: "flex flex-col", children: [_jsx("span", { className: "text-xs font-mono font-semibold text-amber-300 tracking-wider", children: "MODULO ALTERNATIVO \u2014 Sperimentale" }), _jsx("span", { className: "text-[10px] font-mono text-amber-400/70", children: "In uso da mercoled\u00EC 18 marzo 2026" })] })] }), _jsx("div", { className: "flex gap-2", children: [1, 2, 3].map((p) => (_jsxs("button", { onClick: () => setActivePage(p), className: `flex-1 py-2 text-xs font-mono font-semibold rounded-lg border transition-all ${activePage === p
                        ...
                        : "bg-card/40 border-border/30 text-muted-foreground hover:bg-card/60"}`, children: ["PAG ", p] }, p))) }), _jsxs("div", { className: "flex items-center justify-center gap-2", children: [_jsx("button", { onClick: zoomOut, className: "p-1.5 rounded-md border border-border/40 bg-card/60 hover:bg-card/80 transition-all", children: _jsx(ZoomOut, { className: "h-4 w-4 text-muted-foreground" }) }), _jsxs("span", { className: "text-[10px] font-mono text-muted-foreground min-w-[40px] text-center", children: [Math.round(scale * 100), "%"] }), _jsx("button", { onClick: zoomIn, className: "p-1.5 rounded-md border border-border/40 bg-card/60 hover:bg-card/80 transition-all", children: _jsx(ZoomIn, { className: "h-4 w-4 text-muted-foreground" }) }), _jsx("button", { onClick: resetZoom, className: "p-1.5 rounded-md border border-border/40 bg-card/60 hover:bg-card/80 transition-all", children: _jsx(RotateCcw, { className: "h-3.5 w-3.5 text-muted-foreground" }) }), (firFormId || activeDraftId) && (_jsxs(_Fragment, { children: [_jsxs("button", { onClick: () => void handleSaveDraft("draft"), className: "ml-2 px-3 py-2 rounded-lg bg-amber-500/20 border border-amber-500/40 text-amber-300 font-display text-xs tracking-wider hover:bg-amber-500/30 transition-colors flex items-center gap-2", children: [_jsx(Save, { className: "h-4 w-4" }), " BOZZA"] }), _jsxs("button", { onClick: () => void handleSaveDraft("final"), className: "px-3 py-2 rounded-lg bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 font-display text-xs tracking-wider hover:bg-emerald-500/30 transition-colors flex items-center gap-2", children: [_jsx(Save, { className: "h-4 w-4" }), " DEFINITIVO"] })] }))] }), _jsx("div", { ref: containerRef, className: "relative w-full rounded-lg overflow-hidden border border-border/20", style: { touchAction: "none" }, onMouseDown: handleMouseDown, onMouseMove: handleMouseMove, onMouseUp: handleMouseUp, onMouseLeave: handleMouseUp, onTouchStart: handleTouchStart, onTouchMove: handleTouchMove, onTouchEnd: handleTouchEnd, onWheel: handleWheel, children: _jsx("div", { style: {
                        ...
                        cursor: scale > 1 ? (isDragging ? "grabbing" : "grab") : "default",
                                ...
                                return (_jsx("input", { type: field.type === "date" ? "date" : field.type === "time" ? "time" : "text", value: displayValue, readOnly: isCanonicalNumeroField, onChange: (e) => handleChange(field.id, e.target.value), style: {
                                        ...style,
                            ...
                            })] }) }) }), (firFormId || activeDraftId) && (_jsxs("div", { className: "flex justify-end gap-2 pt-2", children: [_jsxs("button", { onClick: () => void handleSaveDraft("draft"), className: "px-5 py-3 rounded-xl bg-amber-500/20 border border-amber-500/40 text-amber-300 font-display text-sm tracking-wider hover:bg-amber-500/30 transition-colors flex items-center gap-2", children: [_jsx(Save, { className: "h-4 w-4" }), " SALVA BOZZA"] }), _jsxs("button", { onClick: () => void handleSaveDraft("final"), className: "px-5 py-3 rounded-xl bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 font-display text-sm tracking-wider hover:bg-emerald-500/30 transition-colors flex items-center gap-2", children: [_jsx(Save, { className: "h-4 w-4" }), " SALVA DEFINITIVO (giacenze)"] })] })), printOnly ? (_jsx("div", { className: "flex justify-end gap-3 pt-2", children: _jsxs("button", { onClick: () => {
