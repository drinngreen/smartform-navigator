...
export function MNFIRFormComplete({ tenantId, mnContext, firFormId, draftData, impiantoId, registryMovementType, enableFatturazione = false, creationMode = false, forceRentriDigital = false }: MNFIRFormCompleteProps) {
  ...
  const handleOpenAssignedFir = (form: any) => {
    ...
    if (!form.trasportatore_targa_rimorchio && (profile as any)?.targa_rimorchio) {
                ...
                {qrCodeData ? (
                  <>
                    ...
                    <img src={qrCodeData} alt="QR Code RENTRI" className="w-72 h-72 object-contain" style={{ imageRendering: 'crisp-edges' }} />
                     <p className="text-gray-500 text-[9px] font-mono text-center max-w-[280px]">Dati di vidimazione RENTRI firmati: Base45, CBOR e COSE_Sign1; verifica anche offline.</p>
                  </>
                ) : (
