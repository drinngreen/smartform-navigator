...
function createPositionedIcons(items) {
...
function savePositions(icons, storageKey) {
    const map = {};
    icons.forEach(ic => { if (ic)
        map[ic.id] = { x: ic.x, y: ic.y }; });
    try {
...
function findFreeCell(targetCol, targetRow, occupied, maxCols = COLS) {
    const key = (c, r) => `${c},${r}`;
...
function initIcons(defs, storageKey) {
...
export function DesktopIconGrid({ icons: iconDefs, storageKey = DEFAULT_STORAGE_KEY }) {
    const navigate = useNavigate();
    const [icons, setIcons] = useState(() => initIcons(iconDefs, storageKey));
    ...
    const handleMouseDown = useCallback((e, icon) => {
        ...
        const handleMouseMove = (me) => {
            ...
            setIcons(prev => prev.map(ic => ic.id === dragRef.current.id
                ? {
                    ...ic,
        ...
        const handleMouseUp = () => {
            ...
            if (!didDrag) {
                ...
                setIcons(prev => {
                    ...
                    const free = findFreeCell(col, row, occupied);
                    const pos = cellToPixel(free.col, free.row);
                    const next = prev.map((ic, idx) => idx === draggedIdx ? { ...ic, x: pos.x, y: pos.y } : ic);
    ...
    return (_jsxs(_Fragment, { children: [_jsx("div", { className: "flex justify-end mb-2", children: _jsx("button", { onClick: resetPositions, className: "text-xs text-white/70 hover:text-white transition-colors font-mono", children: "Reset Desktop" }) }), _jsx("div", { ref: containerRef, className: "relative w-full select-none", style: { minHeight }, children: icons.map((icon) => (_jsx("div", { className: "absolute cursor-grab active:cursor-grabbing group", style: { left: icon.x, top: icon.y, width: 160 }, onMouseDown: (e) => handleMouseDown(e, icon), onContextMenu: (e) => handleContextMenu(e, icon), children: _jsxs("div", { className: "flex flex-col items-center gap-3 p-4 rounded-2xl transition-all duration-300 hover:bg-white/10 hover:shadow-[0_0_30px_rgba(251,191,36,0.2)]", children: [_jsx("div", { className: "w-24 h-24 rounded-2xl flex items-center justify-center overflow-hidden", style: {
                                ...
                                }, children: _jsx("img", { src: icon.iconImage, alt: icon.label, className: "h-12 w-12 transition-transform duration-300 group-hover:scale-200", loading: "eager", decoding: "async", fetchPriority: "high" }) }), _jsx("span", { className: "text-sm text-center font-semibold text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.5)] group-hover:text-white transition-colors", children: icon.label })] }) }, icon.id))) }), _jsx(DesktopIconContextMenu, { iconId: contextMenu?.iconId ?? null, iconLabel: contextMenu?.iconLabel ?? "", position: contextMenu ? { x: contextMenu.x, y: contextMenu.y } : null, subItems: contextMenu?.subItems, onClose: () => setContextMenu(null) })] }));
}
