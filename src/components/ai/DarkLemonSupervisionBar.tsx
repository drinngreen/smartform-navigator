...
export function DarkLemonSupervisionBar({ className }: { className?: string }) {
  ...
  return (
    <div
      className={cn(
        "mx-3 mb-2 flex items-center gap-2 rounded-lg border px-2.5 py-1.5 text-[11px]",
        level === "error"
      ...
      <span className="flex-1 truncate">
        {supervision?.message
          || (entries.length === 0
            ...
            : `${entries.length} azioni monitorate${errors > 0 ? ` • ${errors} con errore` : ""}`)}
      </span>
      <button
        type="button"
        onClick={() => setAutopilot(!autopilot)}
        className={cn(
          "inline-flex items-center gap-1 rounded-md border px-2 py-0.5 font-medium transition-colors shrink-0",
          autopilot
            ? "border-cyan-400/40 bg-cyan-500/20 text-cyan-200"
            : "border-white/15 bg-white/5 text-white/60 hover:text-white",
        )}
        title="Autopilot: l'agente concatena gli strumenti e verifica i risultati da solo"
      >
        {autopilot ? <Gauge className="h-3 w-3" /> : <ShieldCheck className="h-3 w-3" />}
