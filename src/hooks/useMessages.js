...
export function useMessages(partnerId, contextFilter) {
        ...
        try {
            ...
            (messagesData || []).forEach((msg) => {
                ...
                if (otherUserId === user.id)
                    return;
                if (contextFilter && !profileUserIds.has(otherUserId))
                    return;
    ...
    const deleteMessage = useCallback(async (messageId, permanent = false) => {
        if (!user)
            return false;
        try {
    ...
    const getAttachmentUrl = useCallback(async (filePath) => {
