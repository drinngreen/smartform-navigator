...
export function useGlobalOutbox(category?: string) {
  return useQuery({
...
export function useSendGlobalEmail() {
  ...
  return useMutation({
    mutationFn: async (payload: {
      ...
      if (error) throw error;
      if (data && !data.ok) throw new Error(data.error || "Errore invio email");
      return data;
    },
    onSuccess: () => {
    ...
    onError: (err: any) => toast.error("Errore invio: " + err.message),
  });
}
...
export function useSyncInbox() {
  ...
  return useMutation({
    ...
    onError: (err: any) => toast.error("Errore sync: " + err.message),
  });
}
