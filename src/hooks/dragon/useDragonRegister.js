...
export function useDragonRegister(filters) {
    ...
    const { data: movements = [], isLoading } = useQuery({
        ...
        `)
            ...
            if (filters?.cerCode)
                query = query.ilike("cer_code", `%${filters.cerCode}%`);
            if (filters?.causeId)
                query = query.eq("cause_id", filters.causeId);
            if (filters?.status)
    ...
    const consolidate = useMutation({
        mutationFn: async (id) => {
            const { data, error } = await supabase
                .from("dragon_register_movements")
                .update({ status: "CONSOLIDATO", updated_by: user?.id })
                .eq("id", id)
                .eq("status", "BOZZA")
                .select()
                .single();
