...
import { FIR_FIELD_LABELS } from './firStore';
import { safeLocalStorage } from '@/lib/safeStorage';
// ALL fields start empty — NO Global Reco / Multyproget presets
export const mnInitialFIRData = {
    selectedFirNumber: "",
...
const isTestFirNumberMN = (value) => typeof value === "string" && /^(test[\s-]?|skkzr)/i.test(value.trim());
export const useMNFIRStore = create()(persist((set, get) => ({
    ...
    getFilledFields: () => {
        ...
        Object.keys(data).forEach((key) => {
            ...
            if (hasValue)
                filled.push({ field: key, label: FIR_FIELD_LABELS[key], value });
        });
        return filled;
    },
    getFormSummary: () => {
